...
def playGame(socket1, socket2):
    global inGame
    #Prompt for choice
    socket1.send(bytes("Enter Cooperate(C) or Betray(B):", "utf-8"))
    socket2.send(bytes("Enter Cooperate(C) or Betray(B):", "utf-8"))
    ...
    socket1.send(bytes(f"You are sentenced to {time1} year(s).", "utf-8"))
    socket2.send(bytes(f"You are sentenced to {time2} year(s).", "utf-8"))
    
    #Report results to the server
    print(f"Prisoner 1 recieved a sentenced of {time1} year(s)")
    print(f"Prisoner 2 recieved a sentenced of {time2} year(s)")
...
while True:
    ...
    if playerNum == 2 and not inGame:
        ...
        inGame = True
        playerNum = 0
        _thread.start_new_thread(playGame, (clientSocket1, clientSocket2)) 
    
    #Accepts a new client
    clientsocket, address = s.accept();
    print(f"A new prisoner from {address} is trying to enter the game!")
    
    #Queues up a player for a game
    if playerNum < 2 and not inGame:
        playerNum += 1
        #Tells the client they are in the game
        clientsocket.send(bytes(f"You are Prisoner {playerNum}, goodluck...", "utf-8"))
        print(f"Prisoner was assigned the role of Prisoner {playerNum}.")
        ...
        clientsocket.send(bytes(f"Server is full, come back later.", "utf-8"))
